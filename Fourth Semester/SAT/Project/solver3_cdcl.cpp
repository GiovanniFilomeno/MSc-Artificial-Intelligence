...
private:
    ...
    bool build(const std::vector<Clause>& in,int n){
        nvars=n;
        if(n==0 && !in.empty()) return false;

        val.assign(n+1,0);
        reason.assign(n+1,-1);
        level_of.assign(n+1,0);
        activity.assign(n+1,0.0);
        watch.assign(n==0?1:2*n+1,{});
        level_beg={0};

        cls.reserve(in.size());
        for(const Clause& c:in){
            if(c.lit.empty()) return false; 
            for(int l:c.lit) if(l==0 || std::abs(l)>n) return false;

            int id = int(cls.size());
            int w1=0, w2=c.lit.size()>1?1:0;
            cls.push_back({c.lit,w1,w2});
            add_watch(c.lit[w1],id);
    ...
    int propagate(){
        ...
        while(head<trail.size()){ 
            const int falselit = -trail[head++];
            auto &wl = watch[lit2idx(falselit,nvars)];
            for(std::size_t i=0;i<wl.size();){
                ...
                if(c.lit.size()==1){
                    ...
                    if(s==0){ assign(lit,cid); ++i; continue; }
                    if(s==(lit>0?1:-1)){ ++i; continue; }
                    wl[i]=wl.back(); wl.pop_back(); return cid; 
                }

                int *myw = (c.lit[c.w1]==falselit? &c.w1 : (c.lit[c.w2]==falselit? &c.w2 : nullptr));
                if(!myw){ ++i; continue; }
                int other = c.lit[(myw==&c.w1)? c.w2 : c.w1];
                int other_val = val[std::abs(other)];
                if(other_val==(other>0?1:-1)){ ++i; continue; }

                bool moved=false;
                for(int k=0;k<(int)c.lit.size();++k){ if(k==c.w1||k==c.w2) continue; int L=c.lit[k]; int sv=val[std::abs(L)];
                    if(sv==0 || sv==(L>0?1:-1)){
                        *myw=k; add_watch(L,cid);
                        wl[i]=wl.back(); wl.pop_back(); moved=true; break;
                    }
                }
                if(moved) continue; 
    ...
    void assign(int lit,int cid){
        int v = std::abs(lit);
        if(!valid_var(v)){
            DBG(std::cerr<<"[WARN] assign: variable out of range ("<<v<<") - ignored.\n");
            return;
        }
        const int newval = (lit>0?1:-1);
        if(val[v]!=0){
            if(val[v]==newval) return; 
            throw std::runtime_error("Conflicting assignment on an already defined variable");
        }
        val[v]=newval;
        reason[v]=cid;
        level_of[v]=cur_level();
        trail.push_back(lit);
