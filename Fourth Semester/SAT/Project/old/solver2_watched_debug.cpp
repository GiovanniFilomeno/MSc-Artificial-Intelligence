...
#include "solver.hpp" // Contains the Solver, Clause, Result, and Stats definitions
#include <vector>
#include <chrono>
...
public:
    ...
    static inline int lit2idx(int lit, int n_vars) {
        if (lit > 0) return lit;
        // For lit < 0, map -1 to n+1, -2 to n+2, ..., -n_vars to 2*n_vars.
        // Example: n=3. lit=-1 -> idx=4. lit=-2 -> idx=5. lit=-3 -> idx=6.
        // The formula n_vars - lit works:
        // n=3, lit=-1: 3 - (-1) = 4
        // n=3, lit=-2: 3 - (-2) = 5
        // n=3, lit=-3: 3 - (-3) = 6
        // Indices used in 'watch' range from 1 to 2*n_vars.
        // The 'watch' vector has size 2*n_vars + 1, so index 0 is unused.
        return n_vars - lit; 
    }
...
private:
    ...
    bool build(const std::vector<Clause>& in, int n){
        ...
        for(const Clause& c_in : in){
            ...
            // std::vector<bool> seen_vars(num_vars + 1, false); // Unused; remove if tautology logic does not need it.
            bool tautology = false;
            for (int lit_val : c_in.lit) { // Renamed 'lit' to 'lit_val' to avoid shadowing.
                if (tautology) break;
                ...
                // Simplification based on val[var] != 0 is risky before all units are propagated.
                // Handling tautologies (x V -x) and duplicates is safer.
                
                bool already_present = false;
                for(int plit : processed_lits) {
                    if(plit == lit_val) already_present = true;
                    if(plit == -lit_val) tautology = true; // Found x and -x.
                }
                if (tautology) break;
            ...
            if (tautology) {
                ...
                for(int l_val : c_in.lit) std::cerr << l_val << " "; // Use l_val.
                std::cerr << "is a tautology. Skipping." << std::endl;
                continue; 
            }
            
            if (processed_lits.empty() && !c_in.lit.empty()) { // The original clause was not empty, but became empty after simplification.
                 std::cerr << "[DEBUG] Solver2::build: Clause ";
                for(int l_val : c_in.lit) std::cerr << l_val << " "; // Use l_val.
                std::cerr << "became empty after simplification (e.g. x -x x). Potentially UNSAT if not handled by tautology rule." << std::endl;
                // If a clause becomes empty and was not a tautology, the formula is UNSAT.
                // If it was (x -x), it is a tautology. If it was (x x -x), it is also a tautology.
                // If it was (x x) and x becomes false, then it becomes empty.
                // This simplification logic must be careful.
                // For now, an empty non-tautological clause is treated as a problem.
                // If processed_lits is empty, the original clause was empty or tautological.
                // The initial empty-clause check should cover empty input clauses.
                // If a non-empty clause becomes empty here, it was a tautology.
                 return false; // A non-tautological clause that becomes empty is a conflict.
            }
             if (processed_lits.empty() && c_in.lit.empty()){
                // This case is already handled at the beginning of build.
             } else if (processed_lits.empty()){ // A tautology produced an empty list.
                 // Already handled by the tautology continue.
             }
            ...
            // Ensure w1 and w2 are valid if processed_lits is empty; this should not happen here.
            if (processed_lits.empty()) {
                 // This should not happen if empty and tautological clauses are handled correctly.
            ...
            for(int l_val : cls[cid].lit) std::cerr << l_val << " "; // Use l_val.
            ...
            for(int lit_val : cls[cid].lit) { // Use lit_val.
    ...
    int propagate(){
        ...
        // current_trail_pointer is the trail index of the next literal whose consequences must be propagated.
        // Start from the latest decision-level offset because previous levels have already been processed.
        ...
        while(current_trail_pointer < trail.size()){
            ...
            while (i < wl.size()) {
                ...
                for(size_t k=0; k < c.lit.size(); ++k){
                    ...
                    if (k == (size_t)c.w1 || k == (size_t)c.w2) continue; // Cannot be one of the current watches.
    ...
    int pick_var() { 
        ...
        // Fallback if all unassigned variables have activity 0, if none are unassigned,
        // or if best_var was not updated. Negative activity should not happen.
    ...
    {
        if(lvl + 1 >= level_ofs.size()) return;      // Nothing to undo.

        size_t first_idx_next_lvl = level_ofs[lvl + 1];

        while(trail.size() > first_idx_next_lvl) {   // Pop all literals from
            int v = std::abs(trail.back());          // higher levels.
            val[v]    = 0;
            reason[v] = -1;
            trail.pop_back();
        }
        level_ofs.resize(lvl + 1);                   // Keep 0..lvl.
    }

    Result search(){
        std::cerr << "[DEBUG] Solver2::search BEGIN" << std::endl;
        std::cerr << "[DEBUG] Solver2::search: Initial propagate() call." << std::endl;
        if(propagate() != -1) { // Propagate initial assignments at level 0.
        ...
        while(true){
            ...
            size_t current_decision_level_num = level_ofs.size() - 1; // Current level number, e.g. 1, 2, ...
            ...
            while(true){ 
                ...
                if (conflict_cid >=0 && conflict_cid < cls.size()) { // Validate cid.
                    bump_clause(conflict_cid); 
                } else {
                    std::cerr << "[ERROR] Solver2::search: Invalid conflict_cid=" << conflict_cid << " from propagate." << std::endl;
                    // This is a serious error and may indicate a problem in propagate().
                    // Force UNSAT to avoid infinite loops or crashes.
                    return Result::UNSAT;
                }


                while(true){ // Backtrack-and-flip loop.
                    if(level_ofs.size() == 1) { // We are at level 0; level_ofs only contains the initial offset {0}.
                        std::cerr << "[DEBUG] Solver2::search: Conflict at level 0 after backtracking. Formula UNSAT." << std::endl;
                        return Result::UNSAT; 
                    }

                    size_t last_decision_level_num = level_ofs.size() - 1; // Most recent decision-level number.
                    std::cerr << "[DEBUG] Solver2::search: Backtracking from level " << last_decision_level_num << std::endl;

                    // The decision literal for this level is trail[level_ofs[last_decision_level_num]].
                    // Ensure level_ofs[last_decision_level_num] is a valid trail index.
                    if (level_ofs[last_decision_level_num] >= trail.size() && !trail.empty()) { // Added !trail.empty().
                         ...
                         return Result::UNSAT; // Serious error.
                    }
                     if (trail.empty() && level_ofs[last_decision_level_num] > 0) { // Empty trail but nonzero offset.
                        std::cerr << "[ERROR] Solver2::search: Trail empty but decision offset > 0." << std::endl;
                        return Result::UNSAT;
                    }
                    // If the trail is empty and offset is 0, there are no decisions to flip at this level.
                    // This should not happen if level_ofs.size() > 1.

                    int decision_lit_at_this_level = 0;
                    if (!trail.empty() && level_ofs[last_decision_level_num] < trail.size()) { // Added non-empty trail check.
                        ...
                        // Strange case: decision level > 0, but trail empty and offset 0.
                        // This implies that the decision was not put into the trail or the trail was cleared incorrectly.
                        ...
                        return Result::UNSAT;
                    } else if (level_ofs.size() == 1) { // We are at level 0, already handled above.
                         return Result::UNSAT;
                    }


                    int var_of_decision = std::abs(decision_lit_at_this_level);
                     if (var_of_decision == 0 && level_ofs.size() > 1) { // decision_lit_at_this_level was 0 and we are not at level 0.
                    ...
                    // Backtrack to the level before last_decision_level_num.
                    // The new current level will be last_decision_level_num - 1.
                    backtrack(last_decision_level_num - 1); 
                                                            
                    // The decision variable should now be unassigned.
                    // phase[var_of_decision] should contain the phase that caused the conflict.
                    ...
                    if(var_of_decision > 0 && var_of_decision <= num_vars && val[var_of_decision] == 0){ 
                        ...
                        // Modify the decision at the current level: `level_ofs.size() - 1`.
                        // The trail offset for this level is `level_ofs.back()`.
                        // enqueue updates val, phase, reason, and trail.
                        enqueue(flipped_lit, -1); 
                                                  
                        break; 
                    } else {
                        // If the variable is not 0 after backtracking, it was assigned at an earlier level.
                        // The flip is not possible at this level, so keep backtracking.
